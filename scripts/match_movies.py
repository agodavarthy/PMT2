...
def write_movie_mentions(data: pd.DataFrame):
    ...
    movie_mentions.to_csv('../data/redial_dataset/movie_mentions.csv', index=False)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(opt.output):
        os.mkdirs(opt.output) 
    matches.to_csv(opt.output+"movies_merged.csv", index=False, encoding='utf-8')
